...
public class PointSlideUtils {
    private static String TAG = PointSlideUtils.class.getSimpleName();
    ...
    private void OnTouchClick(View v, MotionEvent event){
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                //左滑 右滑 x
                int w = ux1 - dx1;
                Log.i(TAG, "OnTouchClick: yazhou w = "+Math.abs(w));
                Log.i(TAG, "OnTouchClick: yazhou h = "+Math.abs(h));
                if(Math.abs(w) > Math.abs(h)){
                    if (w > 0) {
                        if (eventUpCallBack != null) eventUpCallBack.RightSlide(w);
                    } else {
                        if (eventUpCallBack != null) eventUpCallBack.LeftSlide(w);
                    }
                }else {
                    Log.i(TAG, "OnTouchClick: user gesture is up or down");
                }
                break;
        }
    }
